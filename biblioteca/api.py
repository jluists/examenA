...
class Pedido(db.Model):
    ...
    def import_data_pedido(self, data):
        try:
...
class Item(db.Model):
    ...
    def import_data(self, data):
        ...
        if endpoint != 'get_producto' or not 'id' in args:
            raise ValidationError('URL de producto no valido: ' + data['producto_url'])
        self.producto = Producto.query.get(args['id'])
        if self.producto is None:
            raise ValidationError('URL no valido de producto: ' + data['producto_url'])
        return self
...
def editar_pedido(id):
    ...
    pedido.import_data_pedido(request.json)
    db.session.add(pedido)
    db.session.commit()
    return jsonify({})
